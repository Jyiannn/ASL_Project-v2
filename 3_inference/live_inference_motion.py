...
try:
    ...
    raise FileNotFoundError(f"Missing motion model files inside 'models/'! Ensure LSTM network is trained: {e}")
...
with vision.HandLandmarker.create_from_options(options) as landmarker:
    print("[INFO] Motion Inference Engine Running... Press ESC to quit.")
    while cap.isOpened():
        ret, frame = cap.read()
        if not ret:
            break
        ...
        if local_landmarks is not None and local_world_landmarks is not None:
            ...
            if len(sequence_memory) == SEQUENCE_LENGTH:
                ...
                text_output = f"Motion: {raw_detected_sign} ({motion_conf*100:.1f}%)"
                cv2.putText(frame, text_output, (22, 102), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 0, 0), 3, cv2.LINE_AA)
                cv2.putText(frame, text_output, (20, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 100, 0), 2, cv2.LINE_AA)
            else:
                cv2.putText(frame, f"Buffering Windows: {len(sequence_memory)}/{SEQUENCE_LENGTH}", 
                            (20, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 215, 255), 2, cv2.LINE_AA)
                
            if motion_conf > 0.70 and raw_detected_sign != "":
        ...
        cv2.imshow("Motion Inference", frame)
